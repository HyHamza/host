...
const dbConfig = {
    ...
    user: 'crossgig_usertest2',
    password: '3800380@Hamza',
    database: 'crossgig_drovetest2',
    port: 3306,
...
async function initDatabase() {
    try {
        ...
        await connection.query(`ALTER TABLE bots ADD IF NOT EXISTS dev_number VARCHAR(20);
`);

        await connection.query(`ALTER TABLE bot_requests
ADD COLUMN IF NOT EXISTS deployment_cost INT NOT NULL,
ADD COLUMN IF NOT EXISTS status ENUM('pending', 'approved', 'rejected') DEFAULT 'pending';`)
        await connection.query(`ALTER TABLE coin_transactions
MODIFY COLUMN transaction_type ENUM('deployment', 'dev_share') NOT NULL,
ADD COLUMN IF NOT EXISTS receiver_phone VARCHAR(20) DEFAULT NULL;`)
        // Users table with password and admin role
        ...
        // Bots table for bot information
        await connection.query(`
            CREATE TABLE IF NOT EXISTS bots (
                id INT AUTO_INCREMENT PRIMARY KEY,
                name VARCHAR(255) NOT NULL,
                repo_url VARCHAR(255) NOT NULL,
                deployment_cost INT DEFAULT 0
            )
        `);
        ...
        await connection.query(`ALTER TABLE deployed_apps
ADD COLUMN IF NOT EXISTS deployed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
ADD COLUMN IF NOT EXISTS last_coin_deduction TIMESTAMP NULL;`);
        await connection.query(`CREATE TABLE IF NOT EXISTS maintenance_logs (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        ...
        await connection.query(`ALTER TABLE coin_transactions
ADD COLUMN IF NOT EXISTS app_id INT NULL;`);
        ...
        await connection.query(`
            CREATE TABLE IF NOT EXISTS deployment_env_vars (
                ...
                FOREIGN KEY (deployment_id) REFERENCES deployed_apps(id)
            )
        `);
        ...
        await connection.query(`
            CREATE TABLE IF NOT EXISTS coin_transactions (
                ...
                amount INT NOT NULL,
                transaction_type ENUM('deployment', 'referral_reward', 'deposit') NOT NULL,
                transaction_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP
            )
        `);

        // country code anad etc
...
`);
        await connection.query(`ALTER TABLE users MODIFY COLUMN last_login_ip VARCHAR(45);`)
        await connection.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS name VARCHAR(255);`)
        // Check if the 'country_code' column exists in the 'users' table
        const [countryCodeRows] = await pool.query(`
...
`);
    ...
    `);
        ...
        const [costTable] = await connection.query(`
    SELECT COUNT(*) AS count 
    FROM information_schema.COLUMNS 
    WHERE TABLE_NAME = 'deployed_apps' 
    AND COLUMN_NAME = 'cost'
`);

        if (costTable[0].count === 0) {
            // Column does not exist, add it
            await connection.query(`
        ALTER TABLE deployed_apps ADD COLUMN cost DECIMAL(10, 2) NOT NULL;
    `);
        }

        // Check if the 'last_claim_time' column exists in the 'users' table
        const [claimtable] = await pool.query(`
    SELECT COUNT(*) AS count 
    FROM information_schema.COLUMNS 
    WHERE TABLE_NAME = 'users' 
    AND COLUMN_NAME = 'last_claim_time'
`);

        if (claimtable[0].count === 0) {
            // Column does not exist, add it
            await pool.query(`
        ALTER TABLE users ADD COLUMN last_claim_time DATETIME DEFAULT NULL;
    `);
        }
        ...
        `);
await connection.query(`ALTER TABLE deployed_apps 
ADD COLUMN IF NOT EXISTS status ENUM('deploying', 'configuring', 'building', 'active', 'failed') DEFAULT 'deploying',
ADD COLUMN IF NOT EXISTS error_message TEXT;`)
await connection.query(`ALTER TABLE deployed_apps ADD INDEX IF NOT EXISTS idx_status (status);`)
await connection.query(`ALTER TABLE deployed_apps ADD INDEX IF NOT EXISTS idx_heroku_app_name (heroku_app_name);`)
        // total deployments
        await connection.query(`ALTER TABLE bots ADD COLUMN IF NOT EXISTS total_deployments INT DEFAULT 0;`)
await connection.query(`ALTER TABLE heroku_api_keys ADD INDEX idx_is_active (is_active);`)
await connection.query(`ALTER TABLE heroku_api_keys ADD INDEX idx_last_checked (last_checked);`)
        // First, let's create new tables for API keys and deployment history
        await connection.query(`CREATE TABLE IF NOT EXISTS heroku_api_keys (
    id INT PRIMARY KEY AUTO_INCREMENT,
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        ...
        await connection.query(`CREATE TABLE IF NOT EXISTS deployment_history (
    id INT PRIMARY KEY AUTO_INCREMENT,
    deployment_id INT,
    app_name VARCHAR(255),
    status VARCHAR(50),
    last_checked TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    last_redeployed TIMESTAMP,
    redeployment_count INT DEFAULT 0,
    FOREIGN KEY (deployment_id) REFERENCES deployed_apps(id)
);`);
        // idx
        await connection.query(`CREATE INDEX IF NOT EXISTS idx_deployed_apps_bot_id ON deployed_apps(bot_id);`)
        // Add new table for bot requests
        connection.query(`CREATE TABLE IF NOT EXISTS bot_requests (
    id INT PRIMARY KEY AUTO_INCREMENT,
    name VARCHAR(255) NOT NULL,
    repo_url VARCHAR(255) NOT NULL,
    ...
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
        ...
        connection.query(`ALTER TABLE bots
ADD COLUMN IF NOT EXISTS website_url VARCHAR(255);`)

        // Table for social link:
        connection.query(`CREATE TABLE IF NOT EXISTS bot_social_links (
    id INT PRIMARY KEY AUTO_INCREMENT,
    request_id INT,
...
`);
        // -- Modify bot_requests table to include website
        connection.query(`ALTER TABLE bot_requests 
ADD COLUMN IF NOT EXISTS website_url VARCHAR(255),
ADD COLUMN IF NOT EXISTS last_updated TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP;`)

        // 
        connection.query(`CREATE TABLE IF NOT EXISTS bot_links (
    id INT PRIMARY KEY AUTO_INCREMENT,
    bot_id INT,
    link_type VARCHAR(50),
    url VARCHAR(255),
    FOREIGN KEY (bot_id) REFERENCES bots(id)
        ...
        // First, create the heroku_accounts table
        connection.query(`CREATE TABLE IF NOT EXISTS heroku_accounts (
    id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        ...
        // Create table for purchased accounts
        connection.query(`CREATE TABLE IF NOT EXISTS purchased_heroku_accounts (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    account_id INT NOT NULL,
    purchased_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id),
    FOREIGN KEY (account_id) REFERENCES heroku_accounts(id)
);`);
        await connection.query(`CREATE TABLE IF NOT EXISTS user_devices (
    ...
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
        ...
        connection.query(` CREATE TABLE IF NOT EXISTS bot_request_env_vars (
    id INT PRIMARY KEY AUTO_INCREMENT,
    request_id INT,
    var_name VARCHAR(255) NOT NULL,
...
async function closeWAConnection() {
    if (sock) {
        try {
            await sock.end();
            sock = null;
            console.log('WhatsApp connection closed due to inactivity');
        } catch (error) {
...
app.post('/verify', async (req, res) => {
    ...
    if (Date.now() - verification.timestamp > 30 * 60 * 1000) {
        ...
        // Delete pending device from database
        await pool.query(
    ...
    if (verification.attempts >= 5) {
        ...
        return res.status(400).json({ error: 'Too many failed attempts. Please try again.' });
    }
    ...
    try {
        ...
        await pool.query(
            'UPDATE user_devices SET is_verified = 1 WHERE id = ?',
            [verification.deviceId]
        );
...
// Enhanced saveDeployment function with retry logic
async function saveDeployment(userId, botId, appName, envValues, maxRetries = 3) {
    let attempt = 0;
    let lastError = null;
    ...
    while (attempt < maxRetries) {
        ...
        try {
            ...
            // Set a shorter lock timeout for this transaction
            await connection.query('SET innodb_lock_wait_timeout = 15');

            // First check if this app name already exists
            const [existing] = await connection.query(
                'SELECT id FROM deployed_apps WHERE heroku_app_name = ? FOR UPDATE',
                [appName]
            );
            ...
            // Insert into deployed_apps with minimal locked time
            const [deployResult] = await connection.query(
                'INSERT INTO deployed_apps (user_id, bot_id, app_name, heroku_app_name, status, deployed_at) VALUES (?, ?, ?, ?, ?, NOW())',
                [userId, botId, appName, appName, 'deploying']
            );

            const deploymentId = deployResult.insertId;

            // Increment the total_deployments counter
            await connection.query(
                'UPDATE bots SET total_deployments = total_deployments + 1 WHERE id = ?',
                [botId]
            );

            // Save environment variables if they exist
            if (envValues && typeof envValues === 'object') {
                const envVarPromises = Object.entries(envValues)
            ...
            // If it's a lock timeout error, wait and retry
    ...
    throw new Error(`Failed to save deployment after ${maxRetries} attempts. Last error: ${lastError.message}`);
}
...
app.post('/deploy', isLoggedIn, checkUserCoins, async (req, res) => {
    console.log('Starting deployment process. Request Body:', req.body);
    ...
    try {
        ...
        // Get bot and user information with row locking
        const [userRows] = await connection.query(
            'SELECT id, coins, phone_number FROM users WHERE phone_number = ? FOR UPDATE',
            [req.session.user.phoneNumber]
        );
        ...
        const [botRows] = await connection.query(
            'SELECT deployment_cost, dev_number, name FROM bots WHERE id = ? FOR UPDATE',
            [botId]
        );
        ...
        // Deploy the bot
        console.log('Starting bot deployment with Heroku...');
        ...
        // Process the deployment and financial transactions
        try {
            ...
            // Update deployed_apps with deployment tracking information
            await connection.query(`
                UPDATE deployed_apps 
                SET 
                    deployed_at = CURRENT_TIMESTAMP,
                    last_coin_deduction = NULL,
                    deployment_cost = ?,
                    status = 'active'
                WHERE id = ?
            `, [deploymentCost, result.deploymentId]);

            // Create initial deployment log
            await connection.query(`
                INSERT INTO deployment_logs 
                (deployment_id, user_id, action, details) 
                VALUES (?, ?, 'initial_deployment', ?)
            `, [
                result.deploymentId,
                userRows[0].id,
                JSON.stringify({
                    initial_cost: deploymentCost,
                    dev_share: devShare,
            ...
            res.json({
                ...
                devShare: devShare,
                remainingCoins: userCoins - deploymentCost,
                nextDeduction: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(),
                botName: botRows[0].name
            });

            // Log successful deployment
            console.log(`Successfully deployed bot ${botId} for user ${req.session.user.phoneNumber}`);

        } catch (error) {
            console.error('Transaction error:', error);
            await connection.rollback();

            // Attempt to clean up Heroku app if transaction failed
            try {
                await deleteApp(result.appData.name);
            } catch (deleteError) {
                console.error('Failed to clean up Heroku app:', deleteError);
            }
